...
class BinanceTestnetClient:
    ...
    async def order_status(self, symbol, order_id):
        async with httpx.AsyncClient(timeout=10) as c:
            r = await c.get(f"{self.BASE}/api/v3/order",
                            params=self._signed({"symbol":symbol.upper(),"orderId":order_id,"recvWindow":5000}),
    ...
    async def cancel_order_list(self, symbol, order_list_id=None, list_client_order_id=None):
        ...
        if order_list_id is not None: params["orderListId"] = order_list_id
        elif list_client_order_id: params["listClientOrderId"] = list_client_order_id
        else: raise ValueError("order_list_id or list_client_order_id is required")
        async with httpx.AsyncClient(timeout=10) as c:
