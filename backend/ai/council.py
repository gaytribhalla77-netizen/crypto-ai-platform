    ...
    def deliberate(
        ...
        *,
        ...
        if orderflow:
            ...
            votes.append(AgentVote(
                "orderflow",
                "BUY" if imb > .12 else "SELL" if imb < -.12 else "WAIT",
                min(95, 55 + abs(imb) * 100),
        ...
        if macro:
            ...
            votes.append(AgentVote(
                "macro",
                "WAIT" if ms >= .8 else "BUY" if surprise > .2 else "SELL" if surprise < -.2 else "WAIT",
                80 if ms >= .8 else 60,
                ["macro event state"],
            ))
        if historical:
            ...
            votes.append(AgentVote(
                "historical",
                "BUY" if d == "BULLISH" else "SELL" if d == "BEARISH" else "WAIT",
                float(historical.get("confidence", 50) or 50),
        ...
        if sentiment:
            ...
            votes.append(AgentVote(
                "sentiment",
                "BUY" if sd == "BULLISH" else "SELL" if sd == "BEARISH" else "WAIT",
                float(sentiment.get("confidence", 50) or 50),
        ...
        contradiction = int(
            any(v.action == "BUY" for v in active)
            and any(v.action == "SELL" for v in active)
        )
        return {
            "action": action,
...
class IQ200Council(TradingCouncil):
    ...
    def deliberate_full(self, payload: dict) -> dict:
        ...
        bull = {
            "agent": "bull",
            "action": "BUY" if buy >= sell else "WAIT",
            "arguments": ["aggregate bullish evidence", "trend/liquidity confirmation required"],
        }
        bear = {
            "agent": "bear",
            "action": "SELL" if sell >= buy else "WAIT",
            "arguments": ["aggregate bearish evidence", "downside/invalidation must survive challenge"],
        }
        objections = []
        if base.get("veto"):
