...
class VoiceService:
    """Stateless NLU. Trade intents remain confirmation-gated; security audit
    intents only describe the requested action and never perform a scan here."""
    ...
    async def handle(self, text: str) -> dict:
        ...
        if is_buy or is_sell:
            ...
            return {
                "intent": "trade", "input": norm, "side": "BUY" if is_buy else "SELL",
                "symbol": symbol, "amount_usdt": amount, "missing": missing,
                "requires_confirmation": True,
                "message": (
        ...
        if _contains_any(norm, INFO_WORDS):
            symbol = _find_symbol(norm)
            return {"intent": "info", "input": norm, "symbol": symbol, "missing": [] if symbol else ["symbol"]}
        if _contains_any(norm, PRICE_WORDS):
            symbol = _find_symbol(norm)
            return {"intent": "price", "input": norm, "symbol": symbol, "missing": [] if symbol else ["symbol"]}
        ...
        if _contains_any(norm, NEWS_WORDS):
            symbol = _find_symbol(norm)
            return {"intent": "news", "input": norm, "symbol": symbol, "missing": [] if symbol else ["symbol"]}

        return {"intent": "unknown", "input": norm,
                "message": "Samajh nahi aaya. Price, portfolio, watchlist, history, security audit, ya buy/sell bol ke try karein."}
